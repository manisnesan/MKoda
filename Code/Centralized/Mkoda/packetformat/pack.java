...
public class pack{
	...
	public byte [] packet;//packet in byte format
	public byte pkttype;// packet type
	public byte ID[]=new byte[4];//packet ID
	public byte TTL;//Time to Live
	public byte IP[]=new byte[4];// IP address
	public byte port_no[]=new byte[2];//Port No
	public byte reqrep;//Request or Reply
	public byte [] data;//payload
	public byte [] paylength=new byte[4];//Payload length
	public pack(){
	 ...
	 */
	public pack(byte[] packet, int length, InetAddress IP, int port_no ){
		...
		this.port_no=gfns.convIntBary_2(port_no);//Converts int to byte array of size 2 bytes
	}
	/*Type			: Constructor
	 *Param pkttype	: packet type
	 *param iD	 	: payload length
	 *param tTl		: TTL
	 *param port_no	: Port Number
	...
	public pack(byte pkttype, int iD, byte tTL, InetAddress iP, int portNo, int paylen, byte[] data) {
		...
		ID = new byte[] {(byte)(iD >>> 24),(byte)(iD >>> 16),(byte)(iD >>> 8),(byte)iD};//Converting int to byte array of size 4
		TTL = tTL; 
		IP = iP.getAddress();
		port_no = new byte[] {(byte)(portNo >>> 8),(byte)portNo}; //Convert into to byte array of 2 bytes
		paylength=new byte[] {(byte)(paylen >>> 24),(byte)(paylen >>> 16),(byte)(paylen >>> 8),(byte)paylen};//Convert int to byte array of size 4
